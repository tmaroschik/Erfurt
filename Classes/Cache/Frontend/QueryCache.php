...
class QueryCache {
	 ...
	 * @access		public
	 * @param		\Erfurt\Cache\Backend\QueryCache\Backend			$backend	backend implementation
	 */
	 ...
	 * @access		public
	 * @returns	\Erfurt\Cache\Backend\QueryCache\Backend			$backend	backend implementation
	 */
	...
	public function save($queryString, $resultFormat = "plain", $queryResult, $duration = 0) {
		...
		if (!($this->backend instanceof \Erfurt\Cache\Backend\QueryCache\Null)) {
	...
	public function getMaterializedViewName($subject, $predicate, $object) {
		...
		if (!($subject instanceof \Erfurt\Rdf\Resource)) {
			$subject = null;
		} else {
			$subject = (string)$subject;
		}

		if (!($predicate instanceof \Erfurt\Rdf\Resource)) {
			$predicate = null;
		} else {
			$predicate = (string)$predicate;
		}

		if (!($object instanceof \Erfurt\Rdf\Resource)) {
			$object = null;
		} else {
