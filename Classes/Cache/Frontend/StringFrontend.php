 ...
 * @scope prototype
 */
class StringFrontend extends \Erfurt\Cache\Frontend\AbstractFrontend {
	...
	public function set($entryIdentifier, $string, array $tags = array(), $lifetime = NULL) {
		...
		if (!is_string($string)) throw new \Erfurt\Cache\Exception\InvalidDataException('Given data is of type "' . gettype($string) . '", but a string is expected for string cache.', 1222808333);
		foreach ($tags as $tag) {
