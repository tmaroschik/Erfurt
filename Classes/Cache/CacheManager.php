...
use \Erfurt\Cache\Frontend\FrontendInterface;
...
class CacheManager implements \Erfurt\Singleton {
	...
	protected $cacheConfigurations = array(
		'Default' => array(
			'frontend' => 'Erfurt\Cache\Frontend\VariableFrontend',
			'backend' =>  'Erfurt\Cache\Backend\FileBackend',
			'backendOptions' => array()
		)
	);

	/**
	 * @param \Erfurt\Cache\CacheFactory $cacheFactory
	 * @return void
	 * @author Robert Lemke <robert@typo3.org>
	 */
	public function injectCacheFactory(\Erfurt\Cache\CacheFactory $cacheFactory) {
	 ...
	 *
	 * @param \Erfurt\Cache\Frontend\FrontendInterface $cache The cache frontend to be registered
	 * @return void
	 * @throws \Erfurt\Cache\Exception\DuplicateIdentifierException if a cache with the given identifier has already been registered.
	 * @author Robert Lemke <robert@typo3.org>
	 * @api
	 */
	public function registerCache(\Erfurt\Cache\Frontend\FrontendInterface $cache) {
		$identifier = $cache->getIdentifier();
		if (isset($this->caches[$identifier])) {
			throw new \Erfurt\Cache\Exception\DuplicateIdentifierException('A cache with identifier "' . $identifier . '" has already been registered.', 1203698223);
		}
	 ...
	 * @return \Erfurt\Cache\Frontend\FrontendInterface The specified cache frontend
	 * @throws \Erfurt\Cache\Exception\NoSuchCacheException
	 * @author Robert Lemke <robert@typo3.org>
	 * @api
	 */
	public function getCache($identifier) {
		if ($this->hasCache($identifier) === FALSE) {
			throw new \Erfurt\Cache\Exception\NoSuchCacheException('A cache with identifier "' . $identifier . '" does not exist.', 1203699034);
		}
		if (!isset($this->caches[$identifier])) {
