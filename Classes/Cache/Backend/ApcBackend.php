 ...
 * @scope prototype
 */
class ApcBackend extends \Erfurt\Cache\Backend\AbstractBackend {
	...
	public function __construct($context, array $options = array()) {
		if (!extension_loaded('apc')) throw new \Erfurt\Cache\Exception('The PHP extension "apc" must be installed and loaded in order to use the APC backend.', 1232985414);
	...
	public function set($entryIdentifier, $data, array $tags = array(), $lifetime = NULL) {
		if (!$this->cache instanceof \Erfurt\Cache\Frontend\FrontendInterface) throw new \Erfurt\Cache\Exception('No cache frontend has been set yet via setCache().', 1232986818);
		if (!is_string($data)) throw new \Erfurt\Cache\Exception\InvalidDataException('The specified data is of type "' . gettype($data) . '" but a string is expected.', 1232986825);
		...
		if ($success === TRUE) {
			...
			throw new \Erfurt\Cache\Exception('Could not set value.', 1232986877);
		}
	}
	...
	public function flush() {
		if (!$this->cache instanceof \Erfurt\Cache\Frontend\FrontendInterface) throw new \Erfurt\Cache\Exception('Yet no cache frontend has been set via setCache().', 1232986971);
