 ...
 * @scope prototype
 */
class FileBackend extends \Erfurt\Cache\Backend\AbstractBackend implements \Erfurt\Cache\Backend\PhpCapableBackendInterface {
	 ...
	 *
	 * @param \Erfurt\Cache\Frontend\FrontendInterface $cache The cache frontend
	 * @return void
	 * @author Robert Lemke <robert@typo3.org>
	 */
	public function setCache(\Erfurt\Cache\Frontend\FrontendInterface $cache) {
		parent::setCache($cache);

		$codeOrData = ($cache instanceof \Erfurt\Cache\Frontend\PhpFrontend) ? 'Code' : 'Data';
		$cacheDirectory = $this->environment->getPathToTemporaryDirectory() . 'Cache/' . $codeOrData . '/' . $this->cacheIdentifier . '/';
		if (!is_writable($cacheDirectory)) {
			try {
				\Erfurt\Utility\Files::createDirectoryRecursively($cacheDirectory);
			} catch (\Erfurt\Utility\Exception $exception) {
				throw new \Erfurt\Cache\Exception('The cache directory "' . $cacheDirectory . '" could not be created.', 1264426237);
			}
		}
		if (!is_dir($cacheDirectory)) throw new \Erfurt\Cache\Exception('The cache directory "' . $cacheDirectory . '" does not exist.', 1203965199);
		if (!is_writable($cacheDirectory)) throw new \Erfurt\Cache\Exception('The cache directory "' . $cacheDirectory . '" is not writable.', 1203965200);

		$this->cacheDirectory = $cacheDirectory;
		$this->cacheEntryFileExtension = ($cache instanceof \Erfurt\Cache\Frontend\PhpFrontend) ? '.php' : '';
	}
	 ...
	 * @return void
	 * @throws \Erfurt\Cache\Exception if the directory does not exist or is not writable or exceeds the maximum allowed path length, or if no cache frontend has been set.
	 * @author Robert Lemke <robert@typo3.org>
	 * @api
	 */
	public function set($entryIdentifier, $data, array $tags = array(), $lifetime = NULL) {
		if (!is_string($data)) throw new \Erfurt\Cache\Exception\InvalidDataException('The specified data is of type "' . gettype($data) . '" but a string is expected.', 1204481674);
		...
		if ($result === FALSE) throw new \Erfurt\Cache\Exception('The temporary cache file "' . $temporaryCacheEntryPathAndFilename . '" could not be written.', 1204026251);
		$i = 0;
		$cacheEntryPathAndFilename = $this->cacheDirectory . $entryIdentifier . $this->cacheEntryFileExtension;
		while (!rename($temporaryCacheEntryPathAndFilename, $cacheEntryPathAndFilename) && $i < 5) {
			$i++;
		}
		if ($result === FALSE) throw new \Erfurt\Cache\Exception('The cache file "' . $cacheEntryPathAndFilename . '" could not be written.', 1222361632);
	}
	...
	public function flush() {
		\Erfurt\Utility\Files::emptyDirectoryRecursively($this->cacheDirectory);
	}
	 ...
	 * @author Robert Lemke <robert@typo3.org>
	 * @throws \Erfurt\Cache\Exception if no frontend has been set
	 */
