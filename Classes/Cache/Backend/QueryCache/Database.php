...
class Database extends Backend {
	...
	final public function checkCacheVersion() {
		$result = false;
		try {
			$query = 'SELECT num FROM tx_semantic_cache_query_version';
	 ...
	 *  tx_semantic_cache_query_result, tx_semantic_cache_query_triple, tx_semantic_cache_query_model, tx_semantic_cache_query_rt, tx_semantic_cache_query_rm,
	 *  tx_semantic_cache_query_version. Furthermore there are some indexes which are created.
	...
	final public function createCacheStructure() {
		...
		if (!in_array('tx_semantic_cache_query_result', $existingTableNames)) {
			$columnSpec = array(
			...
			$this->store->createTable('tx_semantic_cache_query_result', $columnSpec);
			$this->store->sqlQuery('CREATE INDEX tx_semantic_cache_query_result_qid ON tx_semantic_cache_query_result(qid)');
			$this->store->sqlQuery('CREATE INDEX tx_semantic_cache_query_result_qid_count ON tx_semantic_cache_query_result(qid,hit_count,inv_count)');
		}

		if (!in_array('tx_semantic_cache_query_triple', $existingTableNames)) {
			$columnSpec = array(
			...
			$this->store->createTable('tx_semantic_cache_query_triple', $columnSpec);
			$this->store->sqlQuery('CREATE INDEX tx_semantic_cache_query_triple_tid ON tx_semantic_cache_query_triple(tid)');
		}
		...
		if (!in_array('tx_semantic_cache_query_rt', $existingTableNames)) {
			...
			$this->store->createTable('tx_semantic_cache_query_rt', $columnSpec);
			$this->store->sqlQuery('CREATE INDEX tx_semantic_cache_query_rt_qid_tid ON tx_semantic_cache_query_rt(qid, tid)');

		}

		if (!in_array('tx_semantic_cache_query_rm', $existingTableNames)) {
			$columnSpec = array(
				'qid' => 'VARCHAR(255)  ' . $vocabulary['col_ascii_bin'] . ' NOT NULL',
				'mid' => 'INT NOT NULL, PRIMARY KEY (qid, mid)',
			);

			$this->store->createTable('tx_semantic_cache_query_rm', $columnSpec);
			$this->store->sqlQuery('CREATE INDEX tx_semantic_cache_query_rm_qid_mid ON tx_semantic_cache_query_rm(qid, mid)');
		}

		if (!in_array('tx_semantic_cache_query_objectkey', $existingTableNames)) {
			$columnSpec = array(
				'qid' => 'VARCHAR(255)  ' . $vocabulary['col_ascii_bin'] . ' NOT NULL',
				'objectkey' => 'VARCHAR(255)  ' . $vocabulary['col_ascii_bin'] . ' NOT NULL, PRIMARY KEY (qid, objectkey)',
			);

			$this->store->createTable('tx_semantic_cache_query_objectkey', $columnSpec);
			$this->store->sqlQuery('CREATE INDEX tx_semantic_cache_query_objectkey_qid_objectkey ON tx_semantic_cache_query_objectkey (qid, objectkey)');
		}

		if (!in_array('tx_semantic_cache_query_version', $existingTableNames)) {
			$columnSpec = array(
				'num' => 'INT NOT NULL PRIMARY KEY',
			);

			$this->store->createTable('tx_semantic_cache_query_version', $columnSpec);
			$this->store->sqlQuery('INSERT INTO tx_semantic_cache_query_version (num) VALUES (1)');
		}
	...
	public function save($queryId, $queryString, $modelIris, $triplePatterns, $queryResult, $duration = 0) {
		...
		if (false === $this->exists($queryId)) {
			...
			$query = "INSERT INTO tx_semantic_cache_query_result (
                qid,
                query,
                result,
                hit_count,
			...
			$queryResult = $this->_encodeResult($queryResult);
			$this->_query("UPDATE tx_semantic_cache_query_result SET result = '" . $queryResult . "', duration = " . $duration . " WHERE qid = '" . $queryId . "'");
		}
	}
	...
	public function load($queryId) {
		$query = "SELECT result FROM tx_semantic_cache_query_result WHERE qid = '" . $queryId . "'";
		...
		switch ($this->store->getBackendName()) {
			...
			case "MySql" :
				$count = $this->_query("SELECT hit_count
                                FROM tx_semantic_cache_query_result
                                WHERE qid='" . $queryId . "' ");
				$this->_query("UPDATE tx_semantic_cache_query_result SET hit_count = " . ($count[0]['hit_count'] + 1) . " WHERE qid='" . $queryId . "' AND result IS NOT NULL");
				break;
			case "Virtuoso":
			default:
				$query = "  UPDATE tx_semantic_cache_query_result
                            SET hit_count = (
                                SELECT hit_count
                                FROM tx_semantic_cache_query_result
                                WHERE qid='" . $queryId . "' ) + 1
		...
		switch ($this->store->getBackendName()) {
			...
			case "MySql" :
				$count = $this->_query("SELECT inv_count
                                FROM tx_semantic_cache_query_result
                                WHERE qid='" . $queryId . "' ");
				$this->_query("UPDATE tx_semantic_cache_query_result SET inv_count = " . ($count[0]['inv_count'] + 1) . " WHERE qid='" . $queryId . "' AND result IS NOT NULL");
				break;
			case "Virtuoso":
			default:
				$query = "  UPDATE tx_semantic_cache_query_result
	...
	public function invalidate($modelIri, $statements = array()) {
		...
		$query = "  SELECT DISTINCT (qid)
                    FROM
                        (
                            SELECT qid qid1
                            FROM tx_semantic_cache_query_rt JOIN tx_semantic_cache_query_triple ON tx_semantic_cache_query_rt.tid = tx_semantic_cache_query_triple.tid
                            WHERE ( " . $clauseString . " )
                        ) first
                    JOIN
                        (
                    ...
                    JOIN
                        tx_semantic_cache_query_result result ON result.qid = qid2
		...
		$query = "  UPDATE tx_semantic_cache_query_result SET result = NULL
                    WHERE
                    (
                        qid IN
                        (
	...
	public function invalidateWithModelIri($modelIri) {
		...
		foreach ($qids as $qid) {
			...
			$query = " SELECT tx_semantic_cache_query_rt.tid tid
                       FROM tx_semantic_cache_query_rt JOIN tx_semantic_cache_query_triple ON tx_semantic_cache_query_rt.tid = tx_semantic_cache_query_triple.tid
                       WHERE ( qid = '" . $qid . "' )";
			$tids = $this->_query($query);
			foreach ($tids as $tid) {
				$tid = $tid['tid'];
				$this->_query("DELETE FROM tx_semantic_cache_query_triple WHERE tid = '" . $tid . "'");
			}

			//delete entries in query_rt
			$this->_query("DELETE FROM tx_semantic_cache_query_rt WHERE qid = '" . $qid . "'");

			//delete entries in query_result
			$this->_query("DELETE FROM tx_semantic_cache_query_result WHERE qid = '" . $qid . "'");
	...
	public function invalidateObjectKeys($oids = array()) {
		...
		foreach ($oids as $oid) {
			...
			$query = "DELETE FROM tx_semantic_cache_query_objectkey WHERE objectkey = '" . $oid . "'";
			$this->_query($query);
		}
	}
	...
	public function getObjectKeys($qids = array()) {
		$oKeys = array();
		foreach ($qids as $qid) {
			$query = "SELECT DISTINCT (objectkey) FROM tx_semantic_cache_query_objectkey WHERE qid='" . $qid . "'";
			;
			$result = $this->_query($query);
			foreach ($result as $entry) {
	...
	public function invalidateAll() {
		$result = $this->store->sqlQuery('SELECT qid FROM tx_semantic_cache_query_result');
		$qids = array();
		foreach ($result as $entry) {
			$qids[] = $entry['qid'];
		}
		$this->store->sqlQuery('UPDATE tx_semantic_cache_query_result SET result = NULL');
		return $qids;
	}
	...
	public function uninstall() {
		...
		$this->store->sqlQuery('DROP TABLE tx_semantic_cache_query_triple');
		$this->store->sqlQuery('DROP TABLE tx_semantic_cache_query_model');
		$this->store->sqlQuery('DROP TABLE tx_semantic_cache_query_result');
		$this->store->sqlQuery('DROP TABLE tx_semantic_cache_query_rt');
		$this->store->sqlQuery('DROP TABLE tx_semantic_cache_query_rm');
		$this->store->sqlQuery('DROP TABLE tx_semantic_cache_query_version');
		$this->store->sqlQuery('DROP TABLE tx_semantic_cache_query_objectkey');
	...
	public function saveTransactions($queryId, $transactions) {
		...
		foreach ($keys as $key) {
			$query = "SELECT * FROM tx_semantic_cache_query_objectkey WHERE qid = '" . $queryId . "' AND objectkey = '" . $key . "'";
			$ret = $this->_query($query);

			if (!(isset($ret[0]['qid']))) {
				$query = "INSERT INTO tx_semantic_cache_query_objectkey (qid, objectkey) VALUES ('" . $queryId . "', '" . $key . "')";
				$this->_query($query);
			}
		}
	...
	public function exists($queryId) {
		$query = 'SELECT * FROM tx_semantic_cache_query_result WHERE qid = \'' . $queryId . '\'';
	...
	public function getUsedTriplePattern($limit = NULL, $minOccurence = NULL) {
		...
		$query = "
            ...
            (
              ...
              FROM
                 tx_semantic_cache_query_rt as rt JOIN
                 tx_semantic_cache_query_result as r  ON (r.qid=rt.qid)
              GROUP BY (rt.tid)
            ) as s1 JOIN
            ...
            (
              ...
              FROM
                tx_semantic_cache_query_triple as t
	...
	public function createMaterializedViews($patternList) {
		...
		foreach ($patternList as $pattern) {
			...
			if ($subject || $predicate || $object) {
				$tableName = "tx_semantic_statement_view_" . $pattern['tid'];
	...
	private function copyStatementsToView($viewName, $subject, $predicate, $object) {
		...
		FROM tx_semantic_statement as s " . $clauseString;
			$result = $this->_query($query);
	...
	public function getMaterializedViews() {
		...
		$viewTables = $this->_query('SHOW TABLES LIKE \'tx_semantic_statement_view%\'');
		if (!$viewTables) {
			return array();
		}
		...
		$query = "SELECT tid, subject, predicate, object FROM tx_semantic_cache_query_triple " . $whereClause;
		$result = $this->_query($query);
		foreach ($result as $entry) {
			$tripleId = $entry['tid'];
	...
	private function _saveTriplePatterns($queryId, $triplePatterns) {
		...
		foreach ($triplePatterns as $triplePattern) {
			...
			$query = "SELECT tid FROM tx_semantic_cache_query_triple WHERE
			...
			if (count($result) == 0) {
				$query = "INSERT INTO tx_semantic_cache_query_triple (
                    subject,
                    predicate,
                    object) VALUES (
			...
			$res = $this->_query("SELECT * FROM tx_semantic_cache_query_rt WHERE qid = '" . $queryId . "' AND tid = '" . $tripleId . "'");
			if (sizeof($res) == 0) {
				$this->_query("INSERT INTO tx_semantic_cache_query_rt (qid, tid) VALUES ('" . $queryId . "','" . $tripleId . "')");
			}
		}
	}
	...
	private function _saveModelIris($queryId, $modelIris) {
		...
		foreach ($modelIris as $modelIri) {
			...
			$this->_query("INSERT INTO tx_semantic_cache_query_rm (qid, mid) VALUES ('" . $queryId . "', '" . $modelId . "')");
		}
	}
	...
	private function _query($sql, $limit = PHP_INT_MAX, $offset = 0) {
		...
		try {
			...
			if (!$this->checkCacheVersion()) {
				...
				catch (\Erfurt\Store\Adapter\Exception $e2) {
				...
				throw new \Erfurt\Exception('Something went wrong with the query cache: ' . $e->getMessage() . ' SQL:' . $sql);
			}
		}
