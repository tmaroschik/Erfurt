 ...
 * @scope prototype
 */
class PdoBackend extends \Erfurt\Cache\Backend\AbstractBackend {
	...
	public function set($entryIdentifier, $data, array $tags = array(), $lifetime = NULL) {
		if (!$this->cache instanceof \Erfurt\Cache\Frontend\FrontendInterface) throw new \Erfurt\Cache\Exception('No cache frontend has been set yet via setCache().', 1259515600);
		if (!is_string($data)) throw new \Erfurt\Cache\Exception\InvalidDataException('The specified data is of type "' . gettype($data) . '" but a string is expected.', 1259515601);
		...
		if ($result === FALSE) throw new \Erfurt\Cache\Exception('The cache entry "' . $entryIdentifier . '" could not be written.', 1259530791);
		...
		foreach ($tags as $tag) {
			...
			if ($result === FALSE) throw new \Erfurt\Cache\Exception('The tag "' . $tag . ' for cache entry "' . $entryIdentifier . '" could not be written.', 1259530751);
		}
	}
	 ...
	 *
	 * @return void
	 * @throws \Erfurt\Persistence\Exception if something goes wrong
	 * @author Karsten Dambekalns <karsten@typo3.org>
	 */
	protected function createCacheTables() {
		try {
			\Erfurt\Utility\PdoHelper::importSql($this->databaseHandle, $this->pdoDriver, FLOW3_PATH_FLOW3 . 'Resources/Private/Cache/SQL/DDL.sql');
		} catch (\PDOException $e) {
			throw new \Erfurt\Persistence\Exception('Could not create cache tables with DSN "' . $this->dataSourceName . '". PDO error: ' . $e->getMessage(), 1259576985);
		}
	}
}
?>