 ...
 * @scope prototype
 */
class RedisBackend extends \Erfurt\Cache\Backend\AbstractBackend {
	 ...
	 * @throws \Erfurt\Cache\Exception if php redis module is not loaded
	 * @author Christopher Hlubek <hlubek@networkteam.com>
	 * @author Christian Kuhn <lolli@schwarzbu.ch>
	 */
	public function __construct($context, array $options = array()) {
		if (!extension_loaded('redis')) {
			throw new \Erfurt\Cache\Exception('The PHP extension "redis" must be installed and loaded in order to use the phpredis redis backend.', 1279462933);
		}
	 ...
	 *
	 * @throws \Erfurt\Cache\Exception if access to redis with password is denied or if database selection fails
	 * @return void
	 * @author Christian Kuhn <lolli@schwarzbu.ch>
	 */
	public function initializeObject() {
		...
		try {
			...
			throw new \Erfurt\Cache\Exception('Could not connect to redis server.', 1294734537, $e);
		}
		...
		if ($this->connected) {
			if (strlen($this->password)) {
				...
				if (!$success) {
					throw new \Erfurt\Cache\Exception('The given password was not accepted by the redis server.', 1279765134);
				}
			}
			...
			if ($this->database > 0) {
				...
				if (!$success) {
					throw new \Erfurt\Cache\Exception('The given database "' . $this->database . '" could not be selected.', 1279765144);
				}
			}
		}
	}
	...
	public function set($entryIdentifier, $data, array $tags = array(), $lifetime = NULL) {
		...
		if (!is_string($data)) {
			throw new \Erfurt\Cache\Exception\InvalidDataException('The specified data is of type "' . gettype($data) . '" but a string is expected.', 1279469941);
		}
