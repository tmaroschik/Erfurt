 ...
 * @scope prototype
 */
class MemcachedBackend extends \Erfurt\Cache\Backend\AbstractBackend {
	...
	public function __construct($context, array $options = array()) {
		if (!extension_loaded('memcache')) throw new \Erfurt\Cache\Exception('The PHP extension "memcache" must be installed and loaded in order to use the Memcached backend.', 1213987706);
	...
	public function initializeObject() {
		if (!count($this->servers)) throw new \Erfurt\Cache\Exception('No servers were given to Memcache', 1213115903);
	 ...
	 *
	 * @param \Erfurt\Cache\Frontend\FrontendInterface $cache
	 * @return void
	 * @author Karsten Dambekalns <karsten@typo3.org>
	 */
	 ...
	 * @return void
	 * @throws \Erfurt\Cache\Exception if no cache frontend has been set.
	 * @throws \InvalidArgumentException if the identifier is not valid or the final memcached key is longer than 250 characters
	 * @throws \Erfurt\Cache\Exception\InvalidDataException if $data is not a string
	 * @author Christian Jul Jensen <julle@typo3.org>
	 * @author Karsten Dambekalns <karsten@typo3.org>
	 * @api
	 */
	public function set($entryIdentifier, $data, array $tags = array(), $lifetime = NULL) {
		...
		if (!$this->cache instanceof \Erfurt\Cache\Frontend\FrontendInterface) throw new \Erfurt\Cache\Exception('No cache frontend has been set yet via setCache().', 1207149215);
		if (!is_string($data)) throw new \Erfurt\Cache\Exception\InvalidDataException('The specified data is of type "' . gettype($data) . '" but a string is expected.', 1207149231);
		...
		try {
			...
			if ($success === TRUE) {
				...
				throw new \Erfurt\Cache\Exception('Could not set value on memcache server.', 1275830266);
			}
		} catch (\Exception $exception) {
			throw new \Erfurt\Cache\Exception('Could not set value. ' . $exception->getMessage(), 1207208100);
		}
	}
	...
	public function flush() {
		if (!$this->cache instanceof \Erfurt\Cache\Frontend\FrontendInterface) throw new \Erfurt\Cache\Exception('Yet no cache frontend has been set via setCache().', 1204111376);
