...
class QueryOptimizer {
	...
	public function optimize(Sparql\Query $query) {
	...
	protected function _optimizeDistinct($query) {
		...
		$sg = new SqlGenerator\Adapter\Ef($query, $this->_engine->getModelIdMapping());
		$ts = new TypeSorter($query, $this->_engine);
		$arSqls = $sg->createSql();
		$ts->setData($sg);
		...
		$sg = new SqlGenerator\Adapter\Ef($tempQuery, $this->_engine->getModelIdMapping());
		$ts = new TypeSorter($tempQuery, $this->_engine);
		$arSqls = $sg->createSql();
		$ts->setData($sg);
		$orderifiedSqls = $ts->getOrderifiedSqls($arSqls);
		...
		#$pattern->addTriplePattern(new Sparql\QueryTriple('?predicate', Erfurt_Rdf_Resource::initWithUri('dummy'), Erfurt_Rdf_Resource::initWithUri('dummy')));
		#    $optionals[] = $pattern;
		foreach ($oldMandatory as $gp) {
			$gp->setTriplePatterns(array());
		...
		foreach ($usedVars as $key => $true) {
			$usedVarsObjects[] = new Sparql\QueryResultVariable($key);
		}
	...
	protected function _queryDb($query, $arSql, $nOffset, $nLimit) {
		$strSql = SqlMerger::getSelect($query, $arSql);
		#echo $strSql;
		if ($strSql === '()') {
			return array();
		}
	...
	protected function _queryMultiple($query, $arSqls) {
		...
		if ($arSM['limit'] === null && $arSM['offset'] === null) {
			...
			$offsetter = new Offsetter($this->_engine, $query);
