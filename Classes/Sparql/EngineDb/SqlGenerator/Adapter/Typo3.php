...
class Typo3 extends EngineDb\SqlGenerator {
	 ...
	 *
	 * @throws EngineDb\SqlGeneratorException   If there is no variable in the result set.
	 */
	public function createSql() {
		...
		switch ($strResultForm) {
			...
			case 'select distinct':
				...
				foreach ($arSelect as $nUnionCount => $arSelectPart) {
					...
					if (count($arSelectPart) == 0
						...
						//throw new EngineDb\SqlGeneratorException('No variable that could be returned.');
					} else {
			...
			case 'count-distinct':
				...
				break;
			default:
				throw new EngineDb\SqlGeneratorException(
	 ...
	 */
	function getTripleSql(Sparql\QueryTriple $triple, Sparql\GraphPattern $graphPattern, $arResultVars) {
		$arSelect = array();
		$strFrom = null;
		$strWhere = null;
		$strWhereEquality = '';
		 ...
		 */
		if (Sparql\Variable::isVariable($subject)) {
		...
		if (Sparql\Variable::isVariable($predicate)) {
		...
		if (Sparql\Variable::isVariable($object)) {
...
//		}
	...
	function getSqlCondition($bject, $strTablePrefix, $strType) {
		if (is_string($bject)) {
			if (Sparql\Variable::isVariable($bject)) {
				//variable?
		...
		if ($bject instanceof Rdf\Resource && $bject->isBlankNode()) {
			//Blank node
			throw new EngineDb\SqlGeneratorException(
			...
			if ($bject instanceof Rdf\Resource) {
				...
				if ($bject instanceof Rdf\Literal) {
					//Literal
	...
	protected function getSqlEqualityCondition($arNew, $arOld) {
		$chTypeNew = $arNew[1];
		$chTypeOld = $arOld[1];
		$strTablePrefixNew = $arNew[0];
		$strTablePrefixOld = $arOld[0];
