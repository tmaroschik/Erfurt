		...
		foreach($subRoutesConfiguration as $subRouteConfiguration) {
			foreach($routesConfiguration as $routeConfiguration) {
				...
				if (!isset($subRouteConfiguration['uriPattern'])) {
					throw new \Erfurt\Configuration\Exception\ParseErrorException('No uriPattern defined in route configuration "' . $name . '".', 1274197615);
		     	}
				...
				if (isset($subRouteConfiguration['defaults'])) {
					$defaults = \Erfurt\Utility\Arrays::arrayMergeRecursiveOverrule($defaults, $subRouteConfiguration['defaults']);
				}
				...
				if (isset($subRouteConfiguration['routeParts'])) {
					$routeParts = \Erfurt\Utility\Arrays::arrayMergeRecursiveOverrule($routeParts, $subRouteConfiguration['routeParts']);
				}
