 ...
 * @scope prototype
 */
class Logger implements \Erfurt\Log\SystemLoggerInterface {
	 ...
	 *
	 * @param \Erfurt\Log\Backend\BackendInterface $backend A backend implementation
	 * @return void
	 * @author Robert Lemke <robert@typo3.org>
	 * @api
	 */
	public function setBackend(\Erfurt\Log\Backend\BackendInterface $backend) {
		$this->backends = new \SplObjectStorage();
	 ...
	 * @api
	 */
	public function addBackend(\Erfurt\Log\Backend\BackendInterface $backend) {
		$this->backends->attach($backend);
		$backend->open();
	}
	 ...
	 * from the logger.
	 *
	 * @param \Erfurt\Log\Backend\BackendInterface $backend The backend to remove
	 * @return void
	 * @throws \Erfurt\Log\Exception\NoSuchBackendException if the given backend is unknown to this logger
	 * @author Robert Lemke <robert@typo3.org>
	 * @api
	 */
	public function removeBackend(\Erfurt\Log\Backend\BackendInterface $backend) {
		if (!$this->backends->contains($backend)) throw new \Erfurt\Log\Exception\NoSuchBackendException('Backend is unknown to this logger.', 1229430381);
		$backend->close();
