 ...
 * @scope prototype
 */
class FileBackend extends \Erfurt\Log\Backend\AbstractBackend {
	...
	public function open() {
		...
		if (file_exists($this->logFileUrl)) {
			...
			if (!is_dir($logPath)) {
				if ($this->createParentDirectories === FALSE) throw new \Erfurt\Log\Exception\CouldNotOpenResourceException('Could not open log file "' . $this->logFileUrl . '" for write access because the parent directory does not exist.', 1243931200);
				\Erfurt\Utility\Files::createDirectoryRecursively($logPath);
			}

			$this->fileHandle = fopen($this->logFileUrl, 'ab');
			if ($this->fileHandle === FALSE) throw new \Erfurt\Log\Exception\CouldNotOpenResourceException('Could not open log file "' . $this->logFileUrl . '" for write access.', 1243588980);
		...
		if ($this->fileHandle === FALSE) throw new \Erfurt\Log\Exception\CouldNotOpenResourceException('Could not open log file "' . $this->logFileUrl . '" for write access.', 1229448440);
	}
