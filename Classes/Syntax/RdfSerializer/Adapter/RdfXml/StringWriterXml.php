...
class StringWriterXml {
	 ...
	 * @var array
	 */
	protected $writtenPrefixes = array();
	...
	public function startElement($ns, $local = null) {
		...
		if ($this->inTag === true) {
			...
			if (($this->wasComment === false) && ($this->level < 2)) {
				$this->linefeed();
			}
		}
		...
		if (count($this->tags) > 0) {
			array_push($this->namespaces, array());
		}
	...
	private function writeQName($ns, $local = null) {
		...
		if (null === $local) {
			$r = \Erfurt\Domain\Resource::initWithUri($ns);
			$ns = $r->getNamespace();
