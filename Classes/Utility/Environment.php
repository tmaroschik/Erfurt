...
class Environment {
	...
	public function getAcceptedFormats() {
		...
		foreach ($flattenedAcceptedTypes as $mimeType => $quality) {
			$format = \Erfurt\Utility\FileTypes::getFilenameExtensionFromMimeType($mimeType);
			if ($format !== '') {
	...
	public function getScriptPathAndFilename() {
		return \Erfurt\Utility\Files::getUnixStylePath($this->SERVER['SCRIPT_FILENAME']);
	}
		...
		foreach ($fieldPaths as $fieldPath) {
			if (count($fieldPath) === 1) {
				...
				foreach ($convolutedFiles[$fieldPath{0}] as $key => $subStructure) {
					$fileInformation[$key] = \Erfurt\Utility\Arrays::getValueByPath($subStructure, array_slice($fieldPath, 1));
				}
			}
			$untangledFiles = \Erfurt\Utility\Arrays::setValueByPath($untangledFiles, $fieldPath, $fileInformation);
		}
	 ...
	 * @throws \Erfurt\Utility\Exception if the temporary directory could not be created or is not writable
	 * @author Robert Lemke <robert@typo3.org>
	 * @author Bastian Waidelich <bastian@typo3.org>
	 */
