...
class Files {
	...
	static public function readDirectoryRecursively($path, $suffix = NULL, $returnRealPath = FALSE, &$filenames = array()) {
		if (!is_dir($path)) throw new \Erfurt\Utility\Exception('"' . $path . '" is no directory.', 1207253462);
	...
	static public function emptyDirectoryRecursively($path) {
		if (!is_dir($path)) throw new \Erfurt\Utility\Exception('"' . $path . '" is no directory.', 1169047616);

		$directoryIterator = new \RecursiveDirectoryIterator($path);
		foreach(new \RecursiveIteratorIterator($directoryIterator) as $fileInfo) {
			if (substr($fileInfo->getFilename(), 0, 1) !== '.' && @unlink($fileInfo->getPathname()) === FALSE) {
				throw new \Erfurt\Utility\Exception('Cannot unlink file "' . $fileInfo->getPathname() . '".', 1169047619);
			}
		}
	...
	static public function createDirectoryRecursively($path) {
		...
		if (!is_dir($path) && strlen($path) > 0) {
			...
			umask($oldMask);
			if (!is_dir($path)) throw new \Erfurt\Utility\Exception('Could not create directory "' . $path . '"!', 1170251400);
		}
	}
	...
	static public function copyDirectoryRecursively($sourceDirectory, $targetDirectory) {
		if (!is_dir($sourceDirectory)) throw new \Erfurt\Utility\Exception('"' . $sourceDirectory . '" is no directory.', 1235428779);

		self::createDirectoryRecursively($targetDirectory);
		if (!is_dir($targetDirectory)) throw new \Erfurt\Utility\Exception('"' . $targetDirectory . '" is no directory.', 1235428779);
