...
class ModelTest extends \Erfurt\Tests\Unit\BaseTestCase {
	...
	public function setUp() {
		$this->_storeStub = new \Erfurt\Domain\StoreStub();
	}

	protected function _getMockedModel() {
		$model = $this->getMock('\Erfurt\Rdf\Model', // original class name
	...
	public function testGetBaseIriWithEmptyBaseReturnsModelIri() {
		$model1 = new \Erfurt\Domain\Graph('http://example.org/');
		$model2 = new \Erfurt\Domain\Graph('http://example.org/', 'http://example.org/resources/');
	...
	public function testUpdateWithMutualDifferenceIssue436DifferentLanguages() {
		...
		$turtleParser = \Erfurt\Syntax\RdfParser::rdfParserWithFormat('turtle');

		$store->importRdf('http://example.org/updateTest/', $turtle1, 'turtle',
						  \Erfurt\Syntax\RdfParser::LOCATOR_DATASTRING, false);
		$statements1 = $turtleParser->parse($turtle1, \Erfurt\Syntax\RdfParser::LOCATOR_DATASTRING);
		$turtleParser->reset();
		$statements2 = $turtleParser->parse($turtle2, \Erfurt\Syntax\RdfParser::LOCATOR_DATASTRING);
