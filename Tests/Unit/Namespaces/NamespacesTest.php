...
namespace Erfurt\Tests\Unit\Namespaces;
/***************************************************************
 *  Copyright notice
 *
...
class NamespacesTest extends \Erfurt\Tests\Unit\BaseTestCase {
	...
	public function setUp() {
		...
		$this->_fixture = new \Erfurt\Namespaces\Namespaces($namespaceOptions);

		$this->_modelStub = new \Erfurt\Tests\Unit\Rdf\ModelStub(self::TEST_GRAPH);
	}

	public function testAddNamespacePrefix() {
		$this->_fixture->addNamespacePrefix($this->_modelStub, 'http://example.com/foo', 'foo');

		$options = $this->_modelStub->getOption(\Erfurt\Namespaces\Namespaces::PREFIX_PREDICATE);
	...
	/**
	 * @expectedException \Erfurt\Namespaces\Namespaces_Exception
	 */
