...
namespace Erfurt\Sparql\Parser\Sparql11;
/***************************************************************
 *  Copyright notice
 *
...
class QueryParser extends \AntlrParser {
	...
	public function __construct($input, $state = null) {
		...
		$this->gSparql11query = new Query\Sparql11query($input, $state, $this);
	...
	public function getGrammarFileName() {
		return "/Users/roll/Documents/workspace/SparqlGrammar/src/Erfurt_Sparql_Parser_Sparql11_Query.g";
	}
	...
	public function groupOrUnionGraphPattern() {
		$this->gSparql11query->groupOrUnionGraphPattern();
	}

	public function limitOffsetClauses() {
		$this->gSparql11query->limitOffsetClauses();
	}

	public function verb() {
		$this->gSparql11query->verb();
	}

	public function relationalExpression() {
		$this->gSparql11query->relationalExpression();
	}

	public function orderClause() {
		$this->gSparql11query->orderClause();
	}

	public function baseDecl() {
		$this->gSparql11query->baseDecl();
	}

	public function unaryExpression() {
		$this->gSparql11query->unaryExpression();
	}

	public function string() {
		$this->gSparql11query->string();
	}

	public function whereClause() {
		$this->gSparql11query->whereClause();
	}

	public function brackettedExpression() {
		$this->gSparql11query->brackettedExpression();
	}

	public function filter() {
		$this->gSparql11query->filter();
	}

	public function groupGraphPatternSub() {
		$this->gSparql11query->groupGraphPatternSub();
	}

	public function blankNodePropertyList() {
		$this->gSparql11query->blankNodePropertyList();
	}

	public function conditionalOrExpression() {
		$this->gSparql11query->conditionalOrExpression();
	}

	public function numericLiteral() {
		$this->gSparql11query->numericLiteral();
	}

	public function solutionModifier() {
		$this->gSparql11query->solutionModifier();
	}

	public function sourceSelector() {
		$this->gSparql11query->sourceSelector();
	}

	public function numericLiteralUnsigned() {
		$this->gSparql11query->numericLiteralUnsigned();
	}

	public function object() {
		$this->gSparql11query->object();
	}

	public function expression() {
		$this->gSparql11query->expression();
	}

	public function numericExpression() {
		$this->gSparql11query->numericExpression();
	}

	public function triplesBlock() {
		$this->gSparql11query->triplesBlock();
	}

	public function iriRef() {
		$this->gSparql11query->iriRef();
	}

	public function booleanLiteral() {
		$this->gSparql11query->booleanLiteral();
	}

	public function constructTemplate() {
		$this->gSparql11query->constructTemplate();
	}

	public function defaultGraphClause() {
		$this->gSparql11query->defaultGraphClause();
	}

	public function constraint() {
		$this->gSparql11query->constraint();
	}

	public function prefixedName() {
		$this->gSparql11query->prefixedName();
	}

	public function graphNode() {
		$this->gSparql11query->graphNode();
	}

	public function objectList() {
		$this->gSparql11query->objectList();
	}

	public function prefixDecl() {
		$this->gSparql11query->prefixDecl();
	}

	public function prologue() {
		$this->gSparql11query->prologue();
	}

	public function datasetClause() {
		$this->gSparql11query->datasetClause();
	}

	public function collection() {
		$this->gSparql11query->collection();
	}

	public function describeQuery() {
		$this->gSparql11query->describeQuery();
	}

	public function subSelect() {
		$this->gSparql11query->subSelect();
	}

	public function offsetClause() {
		$this->gSparql11query->offsetClause();
	}

	public function variable() {
		$this->gSparql11query->variable();
	}

	public function primaryExpression() {
		$this->gSparql11query->primaryExpression();
	}

	public function iriRefOrFunction() {
		$this->gSparql11query->iriRefOrFunction();
	}

	public function selectQuery() {
		$this->gSparql11query->selectQuery();
	}

	public function builtInCall() {
		$this->gSparql11query->builtInCall();
	}

	public function constructQuery() {
		$this->gSparql11query->constructQuery();
	}

	public function additiveExpression() {
		$this->gSparql11query->additiveExpression();
	}

	public function blankNode() {
		$this->gSparql11query->blankNode();
	}

	public function numericLiteralPositive() {
		$this->gSparql11query->numericLiteralPositive();
	}

	public function varOrIRIref() {
		$this->gSparql11query->varOrIRIref();
	}

	public function graphGraphPattern() {
		$this->gSparql11query->graphGraphPattern();
	}

	public function regexExpression() {
		$this->gSparql11query->regexExpression();
	}

	public function optionalGraphPattern() {
		$this->gSparql11query->optionalGraphPattern();
	}

	public function query11() {
		$this->gSparql11query->query11();
	}

	public function graphTerm() {
		$this->gSparql11query->graphTerm();
	}

	public function project() {
		$this->gSparql11query->project();
	}

	public function functionCall() {
		$this->gSparql11query->functionCall();
	}

	public function orderCondition() {
		$this->gSparql11query->orderCondition();
	}

	public function valueLogical() {
		$this->gSparql11query->valueLogical();
	}

	public function namedGraphClause() {
		$this->gSparql11query->namedGraphClause();
	}

	public function triplesNode() {
		$this->gSparql11query->triplesNode();
	}

	public function limitClause() {
		$this->gSparql11query->limitClause();
	}

	public function graphPatternNotTriples() {
		$this->gSparql11query->graphPatternNotTriples();
	}

	public function rdfLiteral() {
		$this->gSparql11query->rdfLiteral();
	}

	public function constructTriples() {
		$this->gSparql11query->constructTriples();
	}

	public function askQuery() {
		$this->gSparql11query->askQuery();
	}

	public function argList() {
		$this->gSparql11query->argList();
	}

	public function conditionalAndExpression() {
		$this->gSparql11query->conditionalAndExpression();
	}
...
QueryParser::$FOLLOW_query11_in_parse37 = new \Set(array());
QueryParser::$FOLLOW_EOF_in_parse39 = new \Set(array(1));
