...
class TypeSorter {
	...
	public function __construct(Sparql\Query $query, $engine) {
		$this->_engine = $engine;
		$this->_query = $query;
	}
	...
	protected function getTypeSets($arSpecialVars, $strFrom, $strWhere, $nUnion) {
		...
		foreach ($arSpecialVars as $strSparqlVar) {
			if (!isset($this->arVarAssignments[$strSparqlVar])) {
				throw new SqlGeneratorException('Variable "' . $strSparqlVar . '" not selected.');
			}
		...
		if ($arResult === false) {
			throw new SqlGeneratorException('Error reading typesets: ' . $sql);
		}
		$arTypes = array();
	...
	function getSqlOrderBy($arTypeSet = array(), $n = 0) {
		...
		if (count($sqlOrder) === 0) {
			throw new \Erfurt\Exception('Something went wrong with ORDER BY.');
		}
